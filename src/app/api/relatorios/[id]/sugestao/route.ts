  ...
  try {
    ...
    for (const ativo of ativosAtuais) {
      ...
      if (research.recomendacao === "sair") {
        movimentacao = "sair";
      } else if (research.recomendacao === "neutro") {
        movimentacao = "reduzir";
      } else if (research.recomendacao === "manter") {
        // Check if over-allocated or not matching objective
    ...
    // Calculate capital freed from exits and reductions
    ...
    for (const m of movimentacoes) {
      ...
      if (m.movimentacao === "sair") {
        ...
        const targetPeso = research?.pct_max_carteira
    ...
    if (totalSlots > 0 && capitalLiberado > 0) {
      // First: increase existing positions to their max
      ...
      // Then: distribute remaining to new entries
      if (novosAtivos.length > 0 && capitalLiberado > 0) {
        // Weight by pct_max_carteira
